...
SOURCE_FOLDER = Path("source_data/raw/salesforce")

ENTITY_FOLDERS = {
    "accounts": "salesforce_accounts",
    "contracts": "salesforce_contracts",
    "contract_rates": "salesforce_contract_rates",
...
def ensure_bronze_tables(cursor):
    for table in ENTITY_FOLDERS.values():
        cursor.execute(f"""
...
def load_file(cursor, file_path, relative_path, table):
    try:
...
def run():
    ...
    try:
        with conn.cursor() as cur:
            ...
            for folder_name, table in ENTITY_FOLDERS.items():
                ...
                for page_file in sorted(entity_folder.glob("*.json")):
                    ...
                    count = load_file(cur, page_file, relative_path, table)
